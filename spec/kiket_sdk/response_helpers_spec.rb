...
RSpec.describe 'KiketSDK Response Helpers' do
  describe '.allow' do
    it 'returns a properly formatted allow response' do
      response = KiketSDK.allow
    ...
    it 'includes message when provided' do
      response = KiketSDK.allow(message: 'Success')

      expect(response[:message]).to eq('Success')
    end

    it 'includes data in metadata' do
      response = KiketSDK.allow(data: { route_id: 123, email: 'test@example.com' })
  ...
  describe '.deny' do
    it 'returns a properly formatted deny response' do
      response = KiketSDK.deny(message: 'Access denied')
  ...
  describe '.pending' do
    it 'returns a properly formatted pending response' do
      response = KiketSDK.pending(message: 'Awaiting approval')
